...
// import IconPicker from "@/components/note_code/IconPicker";
// import { type PickedIcon } from "@/components/note_code/IconPicker";
...
//                   className="h-9 w-9 rounded-xl border border-border/60 bg-background/60 backdrop-blur hover:bg-muted/70 transition p-0"
//                 >
//                   {sidebarOpen ? (
//                     <PanelLeftClose className="h-4 w-4 opacity-80" />
//                   ) : (
//                     <PanelLeftOpen className="h-4 w-4 opacity-80" />
//                   )}
//                 </Button>
...
//                   className="rounded-xl hover:shadow-sm transition"
//                 >
//                   <Plus className="mr-2 h-4 w-4" />
//                   New
//                 </Button>
...
//             {/* MOBILE: Brand + Actions (optional) */}
//             <div className="md:hidden flex items-center justify-between w-full">
//               <div className="ml-2 leading-tight select-none">
//                 <div className="text-sm font-semibold tracking-tight">
//                   BINHLAIG
//                 </div>
...
//             {iconOpen && (
//               //   <IconPicker
//               //     onPick={(txt) => insertToComposer(txt)}
//               //     onClose={() => setIconOpen(false)}
//               //   />

//               <IconPicker
//                 onPick={(picked) => {
//                   setPickedIcon(picked); // ✅ icon ကို state ထဲသိမ်း (စာမထည့်)
//                   setIconOpen(false);
//                 }}
//                 onClose={() => setIconOpen(false)}
//               />
...
import NoteMarkdown from "@/components/note_code/NoteMarkdown";
import { useRouter } from "next/navigation";
import IconPicker, { type PickedIcon } from "@/components/note_code/IconPicker";
...
export default function CodeNotesPage() {
  ...
  // ✅ optional: keep last picked icon (not required)
  const [pickedIcon, setPickedIcon] = useState<PickedIcon | null>(null);
        ...
        <div className="mx-auto max-w-6xl px-4 pb-4">
          <div className="rounded-3xl border bg-background/80 px-3 py-3 shadow backdrop-blur">
            {/* {iconOpen && (
                <IconPicker
                onPick={(picked) => {
                    if (picked?.v) insertToComposer(picked.v);
                    setIconOpen(false);
                  }}
  ...
  <IconPicker
    onPick={(picked) => {
      ...
      if (picked.kind === "emoji") {
        insertToComposer(picked.v);
      }

      if (picked.kind === "lucide") {
        insertToComposer(`:${picked.name}:`);
      }
  ...
  />
            ...
            {pickedIcon ? (
              <div className="mt-2 px-1 text-[11px] text-muted-foreground">
                Last icon: <span className="font-medium">{pickedIcon.label}</span>
              </div>
            ) : null}
          </div>
        </div>
